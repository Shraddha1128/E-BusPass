 ...
 * @author DELL
 */
public class EditDriverForm3 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try
            {
                ...
                PreparedStatement st = con.prepareStatement("update drivers set DriverId=?, DriverName=?, DriverNumber=?, DriverAddress=?, BusRouteNumber=?,BusRoute=?,StartTime=?,EndTime=?, DrivingLicense=? where DriverId='"+did+"' ;");
               st.setString(1, driverId);
                    st.setString(2, drivername);
                    st.setString(3, drivernumber);
                    st.setString(4, driveraddress);
                    st.setString(5, driverbusrouteno);
                    st.setString(6, driverbusroute);
                   st.setString(7, driverstarttime);
                    st.setString(8, driverendtime);
